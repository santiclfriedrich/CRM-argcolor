...
// Un color (hue) por estado, sincronizado entre la LETRA del estado (`badge`,
// SIN fondo — solo color de letra) y el FONDO del nombre del cliente (`chip`,
// tinte del mismo color). Los estados en curso conservan su color; el cierre va
// en verde de tonalidad creciente (pagó → entregado → finalizado).
const ESTADO_COLOR: Record<EstadoOportunidad, { chip: string; badge: string }> = {
  nueva: {
    ...
    badge: "bg-transparent px-0 text-amber-700 dark:text-amber-300",
  },
  requiere_aclaracion: {
    chip: "border-amber-200 bg-amber-100 text-amber-800 dark:border-amber-400/25 dark:bg-amber-400/15 dark:text-amber-200",
    badge: "bg-transparent px-0 text-amber-700 dark:text-amber-300",
      ...
      {data && (
        <div className="mt-4 min-h-0 flex-1 overflow-auto rounded-2xl border border-line">
          ...
          >
            ...
            <thead>
              <tr className="whitespace-nowrap [&_th]:relative [&_th]:sticky [&_th]:top-0 [&_th]:z-10 [&_th]:bg-surface2 [&_th]:px-3 [&_th]:py-2.5 [&_th]:text-left [&_th]:text-xs [&_th]:font-semibold [&_th]:text-ink [&_th]:shadow-[inset_0_-1px_0_var(--c-line)]">
